...
LessFile.prototype = {
  ...
  compile: function(compileOptions, callback) {
    ...
    this._getParser().parse(fs.readFileSync(this.file, 'utf-8'), function(err, tree) {
      if (err) {
        fs.writeFileSync(self.dstFile, generateAetherDebugBarError(err), 'utf-8');
        callback(err);
      } else {
        try {
...
function generateAetherDebugBarError(err) {
    return '#aetherDebugBar { right: 0 !important; display: block !important; }' +
           '#aetherDebugBar div:before { content: "Error in ' + path.basename(err.filename) + ':' + err.line + '"; display: "block"; background: red; }';
}
